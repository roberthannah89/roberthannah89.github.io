...
`scripts/build_hike_gpx.py`. Prefer running that script over
...
A static map generator is at `scripts/make_topo_map.py`:

```bash
~/venvs/dev/bin/python scripts/make_topo_map.py \
