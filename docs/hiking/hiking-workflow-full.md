...
~/venvs/dev/bin/python scripts/validate_images.py --slug <slug>  # single hike
~/venvs/dev/bin/python scripts/validate_images.py --fix          # auto-fix wiki URLs

# Fast path for agents / batch work:
~/venvs/dev/bin/python scripts/add_hike.py \
  --spec /abs/path/to/<slug>.spec.json

# Scaffold a new hike:
~/venvs/dev/bin/python scripts/new_hike.py \
...
~/venvs/dev/bin/python scripts/render_hike.py

# Render single hike (skip index):
~/venvs/dev/bin/python scripts/render_hike.py --slug <slug>

# Probe + validate all URLs (slow, use after major updates):
~/venvs/dev/bin/python scripts/render_hike.py --probe

# Serial rendering (for debugging):
~/venvs/dev/bin/python scripts/render_hike.py --jobs 1
```
...
Spec examples live in `templates/spec-examples/`.
...
The index page (`pages/hikes/index.html`) reads from each `data.json`:
