...
Usage
-----
    python render_hike.py                              # render all hikes under pages/hikes/routes
    python render_hike.py --root pages/hikes/routes
...
DEFAULT_ROOT = REPO_ROOT / "pages" / "hikes" / "routes"
...
def render_index(root: Path, hikes: list[dict]) -> tuple[Path, float]:
    ...
    Writes to parent directory so index.html lives at pages/hikes/ not pages/hikes/routes/
    """
    t0 = time.perf_counter()
    env = _make_env()
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--root", type=Path, default=DEFAULT_ROOT,
                   help="Directory to scan for *.data.json (recursive). Default: pages/hikes/routes")
