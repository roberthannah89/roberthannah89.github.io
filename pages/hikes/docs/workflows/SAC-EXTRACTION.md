...
- ✅ `scripts/scrape_sac_route_page.py` — reads the authenticated route page HTML (from a local file or via `--url` with the saved cookie) and extracts: `og:title`, `og:image`, `og:description`, difficulty, ascent/descent time + gain/drop, departure point name + elevation + transport mode, segment list, photo gallery (`/processed/…` URLs) with per-photo caption/photographer/date parsed from figcaptions, and the sidebar route-author box. `--apply --slug <slug>` patches `routes/<slug>/<slug>.data.json` in place, only replacing fields whose value still starts with `"TODO"`. Pass `--replace-non-todo` to overwrite existing values too. Photos are written as `{"url", "lightbox_url", "alt", "caption_html", "copyright", "credit_date"}`; route author as `{"name", "bio_html", "portrait_url"}`. On re-scrape the apply merges per-photo credits into existing photos by URL match, so old hikes gain attribution without losing manual curation.
- ✅ `scripts/backfill_sac_credits.py` — one-shot batch: walk every hike, re-scrape the SAC route page for any with `sac-cas.ch/processed/` photos missing `copyright`. Dry-run by default (lists what would run); pass `--apply` to fetch + patch. Sleeps 1 s between requests.
