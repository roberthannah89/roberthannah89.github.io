...
hikes/
  routes/                        # One folder per hike
    ...
    zindlenspitz/                # Example with SAC source data
      ...
      sac-route-4567.json        # Raw SAC API capture (temporary — delete after data.json complete)
  scripts/
    ...
    inspect_sac_json.py          # print SAC JSON structure (photos, segments, metadata)
    check_gpx_gaps.py            # verify GPX track connectivity
  templates/
...
| `sac-route-<ID>.json` | Captured via Playwright from SAC API | Temporary — delete after `data.json` is fully populated |

> [!NOTE]
> All route-specific files live in the route folder — never in the repo root. The SAC JSON is kept alongside the data it populates until extraction is complete, then deleted.
