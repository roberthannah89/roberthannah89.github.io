...
function init(el) {
  panelEl = el;
  // Close-button click is delegated here (instead of an inline onclick that
  // would need a window global), so the module doesn't have to leak out of
  // the module scope just for the close affordance.
  panelEl.addEventListener('click', function (e) {
    var target = e.target;
    if (target && target.classList && target.classList.contains('panel-close')) {
      close();
    }
  });
...
function render(poi) {
  ...
  // Close button uses a delegated click listener on the panel (see init).
  panelEl.innerHTML = '<button class="panel-close" type="button" title="Close">&times;</button>' + html;
}
