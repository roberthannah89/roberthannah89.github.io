...
import { WebcamLayer } from './webcams.js';
...
function initMap() {
  ...
  if (window.MapShared && window.MapShared.addLayerControl) {
    window.MapShared.addLayerControl(map, { defaultLayer: 'hike' });
...
function createMarkers(routes) {
  routes.forEach(function (poi) {
    ...
    var hasPage = !!(SidePanel.matchingHike && SidePanel.matchingHike(poi));
...
function boot() {
  ...
  Filters.loadState(UrlSync.readFromUrl());
