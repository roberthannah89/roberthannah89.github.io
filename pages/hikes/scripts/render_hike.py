...
def _grade_pill_class(grade: str) -> str:
    ...
    Keep in sync with ``gradeClass`` in ``templates/_assets/hike_map/side_panel.js``.
    """
...
def render_index(root: Path, hikes: list[dict]) -> tuple[Path, float]:
    ...
    # by templates/_assets/hike_map/side_panel.js to surface the hero image +
    # local page link inside the expanded route view. Keep it small — JSON only.
