...
- [hike-html-page.md](./hike-html-page.md) — **preferred deliverable**: rich
  ...
  live with the content at `/opt/code/hikes/{scripts,templates}/`. Per-hike
  content is `/opt/code/hikes/<slug>/<slug>.data.json`.
...
generated `<slug>.html` or `index.html`. Tooling lives in the hikes repo at
`/opt/code/hikes/`, not in this skill.

```bash
cd /opt/code/hikes

# 1. Build GPX + track.js (one-time per hike):
python scripts/build_hike_gpx.py --slug <slug> --peak <Peak> \
    --trailhead <Village> --via <Wpt1> --via <Wpt2> --bbox <s,w,n,e> \
    --out-dir <slug>/

# 2. Author <slug>/<slug>.data.json (copy from an existing hike).
#    Validated against templates/hike_data.schema.json on render.

# 3. Render (parallel; auto-renders index.html and copies _assets/):
python scripts/render_hike.py
python scripts/render_hike.py --probe          # also HEAD-check URLs
python scripts/render_hike.py --slug <slug>    # single hike, no index
```
